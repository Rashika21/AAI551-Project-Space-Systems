...
def create_orbit_animation(satellite, simulation_hours=2, fps=30, trail_length=50):
    ...
    num_frames = int(fps * simulation_hours * 60)  # Total frames
    time_step = total_seconds / num_frames
    ...
    def animate(frame):
        ...
        time_text.set_text(f'Time: {hours:02d}h {minutes:02d}m {seconds:02d}s')
        
        # Calculate current altitude
        distance = np.sqrt(x**2 + y**2 + z**2)
        altitude = distance - earth_radius
        info_text.set_text(f'Altitude: {altitude:.1f} km')
        
        # Update title
        ax.set_title(f'{satellite.name} Orbit Animation\n'
                    f'Simulation: {simulation_hours}h | '
                    f'Period: {orbital_period/60:.1f} min',
...
def create_multi_satellite_animation(satellites, simulation_hours=2, fps=30, trail_length=50):
    ...
    total_seconds = simulation_hours * 3600
    num_frames = int(fps * simulation_hours * 60)
    times = np.linspace(0, total_seconds, num_frames)
    ...
    def animate(frame):
        ...
        ax.set_title(f'Multi-Satellite Orbit Animation\n'
                    f'Simulation: {simulation_hours}h | Satellites: {len(satellites)}',
                    fontsize=14, fontweight='bold')
